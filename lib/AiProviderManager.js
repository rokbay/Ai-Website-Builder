...
class AiProviderManager {
    ...
    async generateLocalResponse(messages, model, workspaceId, updateMessages) {
        ...
        try {
            const formattedMessages = messages.map(m => ({ role: m.role === 'ai' ? 'assistant' : m.role, content: m.content }));
            formattedMessages.unshift({ role: 'system', content: Prompt.CODE_GEN_PROMPT });
