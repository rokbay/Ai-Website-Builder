...
class AiProviderManager {
    ...
    _calculatePayloadSize(messages) {
        try {
            const byteSize = new Blob([JSON.stringify(messages)]).size;
            return {
                byteSize,
                estimatedTokens: Math.round(byteSize / 4)
            };
        } catch (error) {
            console.error('Error calculating payload size:', error);
            return { byteSize: 0, estimatedTokens: 0 };
        }
    }

    async getResponse({ messages, model, workspaceId, streamAction, updateMessages }) {
        // Dispatch payload telemetry before request
        const metrics = this._calculatePayloadSize(messages);
