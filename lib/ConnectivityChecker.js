...
class ConnectivityChecker {
    ...
    /**
     * Test connectivity with current strategy
     */
    async testConnection() {
        notify.connectivityCheck('checking connection');

        for (const strategy of this.strategies) {
            try {
                notify.status(`Trying ${strategy.name}...`, 'info');
                const result = await strategy.test();

                if (result.success) {
                    this.currentStrategy = strategy;
                    this.isConnected = true;
                    notify.connectivitySuccess(strategy.name);
                    notify.status(`Connected via ${strategy.name}`, 'success');
                    return result;
                }
            } catch (error) {
                const reason = error.message || 'Connection failed';
                notify.connectivityFailed(strategy.name, reason);
                continue;
            }
        }
