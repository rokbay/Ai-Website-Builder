...
export class AIProviderFactory {
    ...
    static async getGoogleStream(prompt, config) {
        ...
        if (!apiKey) throw new Error("Cloud API Key (GEMINI_API_KEY or NEXT_PUBLIC_GEMINI_API_KEY) was not found in environment.");
        ...
        const contents = Array.isArray(prompt)
            ? prompt.map(m => ({ role: m.role === 'ai' ? 'model' : 'user', parts: [{ text: m.content }] }))
    ...
    static async getLocalStream(prompt, config, baseUrl) {
        try {
            const messages = Array.isArray(prompt)
                ? prompt.map(m => ({ role: m.role === 'ai' ? 'assistant' : m.role, content: m.content }))
